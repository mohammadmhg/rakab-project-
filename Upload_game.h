...
public:
    ...
    void uploading_gamplay_data(const int,vector<Gameplay>&,Rish_Sefid&,Handle_Data&);
    void uploading_control_data(const int,Control_Cards&);
    void uploading_cities_data(const int,War_Sign &,Peace_Sign &,vector <City> &);
    void uploading_red_cards_data(const int,vector <City>&);
